...
# Définition des "Profils" de clients via des tickers de différents pays
# Chaque liste représente un worker avec une "histoire" géographique différente
countries = {
    "Worker_USA": ['AAPL', 'GOOGL', 'MSFT', 'JPM'],       # Apple, Google, Microsoft, JPMorgan
    "Worker_France": ['MC.PA', 'OR.PA', 'TTE.PA', 'AIR.PA'], # LVMH, L'Oréal, TotalEnergies, Airbus
    "Worker_Germany": ['SAP.DE', 'VOW3.DE', 'SIE.DE', 'DTE.DE'], # SAP, Volkswagen, Siemens, Deutsche Telekom
    "Worker_Japan": ['7203.T', '6758.T', '9984.T', '8306.T'], # Toyota, Sony, SoftBank, Mitsubishi UFJ
    "Worker_UK": ['HSBA.L', 'AZN.L', 'ULVR.L', 'BP.L'] # HSBC, AstraZeneca, Unilever, BP
}

def get_client_data(tickers, start='2020-01-01', end='2023-01-01'):
    """Télécharge les données et calcule les log-rendements pour une liste de tickers."""
    # Téléchargement
...
for name, tickers in countries.items():
    ...
    entropy_results_for_json[name] = {"entropy": entropy, "normalized_entropy": normalized_entropy, "bins": BINS}
...
json_path = os.path.join(output_dir, "finance_country_entropy_analysis.json")
...
plt.title("Visualisation du Non-IID : Densité des rendements par pays")
